...
public class FileStorage {
    ...
    /** A file id sent by the client must be one of this circle's uploads. Null stays null. */
    public String owned(Long organizationId, String fileId, String field) {
        if (fileId == null || fileId.isBlank()) {
            return null;
        }
        return files.findByIdAndOrganizationId(fileId, organizationId)
            .map(StoredFile::getId)
            .orElseThrow(() -> ApiException.field(field, "file.missing"));
    }
