...
import SyncStatus from '@/components/care/SyncStatus.vue'
...
const main = computed(() => items.value.filter((i) => !i.menu))
const menu = computed(() => items.value.filter((i) => i.menu))
...
onMounted(() => {
  if (auth.user) sync.start(auth.user.id)
})
          ...
          <div ref="userRoot" class="user">
            <button type="button" class="user__btn" :aria-expanded="userOpen" aria-haspopup="menu" @click="userOpen = !userOpen">
              ...
              <span class="user__who">
                ...
                <span class="user__org">{{ auth.user ? $t(`roles.${auth.user.role}`) : '' }} · {{ auth.organization?.name }}</span>
              </span>
            ...
            <Transition name="drop">
              <div v-if="userOpen" class="user__menu" role="menu">
                ...
                <RouterLink v-for="i in menu" :key="i.name" :to="{ name: i.name }" class="user__item" role="menuitem">
...
/* The sections sit on their own row under the brand: eight of them, with room to breathe */
.band__nav {
  order: 3;
  display: flex;
  gap: 4px;
