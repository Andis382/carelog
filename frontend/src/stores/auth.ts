...
export const useAuth = defineStore('auth', () => {
  ...
  async function join(payload: { token: string; name: string; email: string; phone: string; password: string; locale: Locale }) {
  ...
  /** Anyone but a viewer may write to the log. */
  const canRecord = computed(() => hasRole('OWNER', 'FAMILY', 'CARER'))
  /** Owner and family plan care: medicines, other people's shifts, the elder's profile. */
  const canPlan = computed(() => hasRole('OWNER', 'FAMILY'))
  const isOwner = computed(() => hasRole('OWNER'))

  return { user, organization, demo, ready, signedIn, apply, clear, load, login, register, join, logout, hasRole, canRecord, canPlan, isOwner }
})
