...
import { PhEnvelopeSimple, PhLinkBreak, PhLockSimple, PhPhone, PhUser } from '@phosphor-icons/vue'
  ...
  <AuthLayout>
    ...
    <div v-else class="stack stack-lg">
      ...
      <form class="stack" novalidate @submit.prevent="submit">
        ...
        <UiField id="f-phone" :label="$t('common.phone')" :hint="$t('auth.joinPhoneHint')" :error="form.error('phone')" optional>
          <template #default="{ id, describedby, invalid: bad }">
            <UiInput :id="id" v-model="form.data.phone" type="tel" inputmode="tel" autocomplete="tel" :icon="PhPhone" :invalid="bad" :describedby="describedby" />
          </template>
        </UiField>
