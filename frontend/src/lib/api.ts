...
export class ApiError extends Error {
  ...
  /** Structured extras some errors carry, e.g. who already gave a dose and when. */
  readonly details: Record<string, unknown>

  constructor(status: number, message: string, errors: FieldErrors = {}, details: Record<string, unknown> = {}) {
    super(message)
    this.name = 'ApiError'
    this.status = status
    this.errors = errors
...
async function request<T>(method: string, url: string, body?: unknown, options: Options = {}, retried = false): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(response.status, message, (data && data.errors) || {}, (data && data.details) || {})
  }
  return data as T
}
